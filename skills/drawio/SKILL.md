...
2. **Desktop-enhanced** — when draw.io Desktop is available, use it for PNG/PDF/JPG export and embedded `.drawio.*` artifacts. Desktop detection checks `DRAWIO_CMD` env var first; set it to the draw.io executable path if installed at a non-standard location (e.g. `C:\Soft\draw.io\draw.io.exe`).
